...
Async.auto(
    {
        ...
        setupRootUser: [
            'rootPassword',
            (results, done) => {
                const sequelize = results.testDB;
                const Account = sequelize.import('./server/models/account');
                const Admin = sequelize.import('./server/models/admin');
                const AdminGroup = sequelize.import(
                    './server/models/admin-group'
                );
                sequelize.import(
                    './server/models/admin-group-permission-entry'
                );
                ...
                Async.auto(
                    {
                        ...
                        linkUser: [
                            ...
                            'createAdmin',
                            function (iresults, done) {
                                iresults.createAdmin
                                    .setUser(iresults.createUser)
                                    .then(
                                        () => {
                                            done(null);
                                        },
